...
const trophy = "https://assets.ccbp.in/frontend/react-js/match-game-trophy.png";
...
class MatchGame extends Component {
  state = {
    ...
    score: 0,
    randomPreviousCatId: imagesList[0].id,
    timer: 60,
    lost: false,
  };
  ...
  onClickImage = (CategoryId) => {
    const { randomPreviousCatId } = this.state;
    const lenImageList = imagesList.length - 1;
    const randomNumber = Math.floor(Math.random() * lenImageList);
    if (CategoryId === randomPreviousCatId) {
      this.setState((prevState) => ({ score: prevState.score + 1 }));
      this.setState({ randomPreviousCatId: imagesList[randomNumber].id });
      this.setState({
        imageUrl: imagesList[randomNumber].imageUrl,
        category: imagesList[randomNumber].category,
      });
    } else {
  ...
  onClickCategory = (TabIdValue) => {
    this.setState({ isFilteredTab: true, tabID: TabIdValue });
  };

  initialImagesList = () => {
    const initialFilteredList = imagesList.filter(
  ...
  render() {
    const { isFilteredTab, tabID, imageUrl, category, score, timer, lost } =
      this.state;
    timer <= 0 && this.componentWillUnmount();
    const filteredListWithTabID = imagesList.filter(
      (eachItem) => eachItem.category === tabID
    );
    const initialFilteredList = this.initialImagesList();
    const initialImagesList = isFilteredTab
      ? filteredListWithTabID
      : initialFilteredList;

    return (
      <div className="app-container">
        <div className="sub-container">
          <NavBar score={score} timerInSeconds={timer} />
          {timer > 0 && lost === false ? (
            <div className="img-container">
              <img className="main-image" alt={category} src={imageUrl} />
              <ul className="ul-tabList">
            ...
            <div className="success-container">
              <img className="success-img" alt="trophy" src={trophy} />
              ...
              >
                <img className="paly-img" alt="reset" src={reset} />
                PLAY AGAIN
              </button>
            </div>
          )}
